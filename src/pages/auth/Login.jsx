...
export default function Login() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!profile.role_confirmed && profile.role !== 'admin') {
        // Both OAuth buttons route through /continue-with, which always asks
        // client/advocate before firing the redirect — so intent is set on
        // the normal path. /choose-role below is just a safety net for the
        // rare case it's missing (e.g. a stale/direct OAuth redirect).
  ...
  // Role is chosen on /continue-with, before the OAuth redirect fires — not
  // here. That page calls oauthLogin itself once a role is picked.
  function handleOAuth(provider) {
    navigate('/continue-with', { state: { provider } });
  }
